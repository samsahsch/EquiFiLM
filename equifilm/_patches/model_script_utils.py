...
def configure_model(
    ...
    # FiLM charge conditioning (with ablation flags)
    charge_film_config = None
    if hasattr(args, "charge_film") and args.charge_film is not None:
        use_mult = not getattr(args, "charge_film_no_mult", False)
        use_add = not getattr(args, "charge_film_no_add", False)
        zero_init = not getattr(args, "charge_film_no_zero_init", False)
        charge_film_config = {
            "mlp_hidden": args.charge_film,
            "use_multiplicative": use_mult,
            "use_additive": use_add,
            "zero_init": zero_init,
        }
        logging.info(
            f"FiLM charge conditioning: mlp_hidden={args.charge_film}, "
            f"mult={use_mult}, add={use_add}, zero_init={zero_init}"
        )
    if model_config is not None:
        model_config["charge_film_config"] = charge_film_config
    if model_config_foundation is not None:
        model_config_foundation["charge_film_config"] = charge_film_config
