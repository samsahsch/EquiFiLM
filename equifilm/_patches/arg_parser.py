...
def build_default_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--charge_film",
        help="Enable FiLM conditioning on total_charge. Pass MLP hidden dim (e.g. 64)",
        type=int,
        default=None,
    )
    parser.add_argument(
        "--charge_film_no_mult",
        help="Ablation: disable multiplicative gating in FiLM (additive only)",
        action="store_true",
    )
    parser.add_argument(
        "--charge_film_no_add",
        help="Ablation: disable additive shift in FiLM (multiplicative only)",
        action="store_true",
    )
    parser.add_argument(
        "--charge_film_no_zero_init",
        help="Ablation: disable zero-init (random init breaks identity-at-start)",
        action="store_true",
    )
    parser.add_argument(
        "--embedding_specs",
