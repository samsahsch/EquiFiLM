...
class MACE(torch.nn.Module):
    def __init__(
        ...
        charge_film_config: Optional[Dict[str, Any]] = None,
    ):
        super().__init__()
        self.charge_film_config = charge_film_config
        ...
        # Attach FiLM conditioning blocks to interactions if configured
        if charge_film_config is not None:
            from mace.modules.blocks import ChargeFiLMBlock

            film_hidden = charge_film_config.get("mlp_hidden", 64)
            use_mult = charge_film_config.get("use_multiplicative", True)
            use_add = charge_film_config.get("use_additive", True)
            zero_init = charge_film_config.get("zero_init", True)
            for inter in self.interactions:
                if hasattr(inter, "conv_tp"):
                    irreps_mid = inter.conv_tp.irreps_out
                    num_scalars = sum(
                        mul for mul, ir in irreps_mid if ir.l == 0
                    )
                    inter.charge_film = ChargeFiLMBlock(
                        num_scalar_channels=num_scalars,
    ...
    def forward(
        ...
        # Charge conditioning: broadcast per-graph charge to per-node
        charge_conditioning = None
        if getattr(self, "charge_film_config", None) is not None and "total_charge" in data:
            # total_charge: [n_graphs] -> [n_nodes, 1]
            charge_per_node = data["total_charge"][data["batch"]].unsqueeze(-1)
            # Normalize: q_norm = charge / n_atoms_per_graph
            _, counts = torch.unique(data["batch"], return_counts=True)
            n_atoms = counts[data["batch"]].unsqueeze(-1).to(charge_per_node.dtype)
            charge_conditioning = charge_per_node / n_atoms
        ...
        for i, (interaction, product) in enumerate(
            ...
            if hasattr(interaction, "charge_film"):
                inter_kwargs["charge_conditioning"] = charge_conditioning
            node_feats, sc = interaction(**inter_kwargs)
            if is_lammps and i == 0:
                node_attrs_slice = node_attrs_slice[: lammps_natoms[0]]
            node_feats = product(
...
class ScaleShiftMACE(MACE):
    ...
    def forward(
        ...
        # Charge conditioning for FiLM (ScaleShiftMACE)
        charge_conditioning = None
        if getattr(self, "charge_film_config", None) is not None and "total_charge" in data:
            charge_per_node = data["total_charge"][data["batch"]].unsqueeze(-1)
            _, counts = torch.unique(data["batch"], return_counts=True)
        ...
        for i, (interaction, product) in enumerate(
            ...
            if hasattr(interaction, "charge_film"):
                inter_kwargs_ss["charge_conditioning"] = charge_conditioning
            node_feats, sc = interaction(**inter_kwargs_ss)
            if is_lammps and i == 0:
                node_attrs_slice = node_attrs_slice[: lammps_natoms[0]]
            node_feats = product(
                node_feats=node_feats, sc=sc, node_attrs=node_attrs_slice
