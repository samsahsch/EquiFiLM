...
class ChargeFiLMBlock(torch.nn.Module):
    """
    Feature-wise Linear Modulation (FiLM) conditioned on charge density.

    Modulates only the scalar (l=0) channels of equivariant features to
    preserve E(3) equivariance. Higher-order (l>0) channels pass through
    unchanged.

    Architecture:
        q_norm -> MLP -> (gamma, beta)
        x_scalar = (1 + gamma) * x_scalar + beta

    Initialization: gamma=0, beta=0 so FiLM acts as identity at start,
    ensuring fine-tuning from a pre-trained model starts from the same
    function and gradually learns charge dependence.
    """
    ...
    def __init__(
        ...
        if use_multiplicative:
            self.gamma_net = torch.nn.Sequential(
                torch.nn.Linear(1, mlp_hidden),
                torch.nn.SiLU(),
                torch.nn.Linear(mlp_hidden, num_scalar_channels),
            )
            if zero_init:
                torch.nn.init.zeros_(self.gamma_net[-1].weight)
                torch.nn.init.zeros_(self.gamma_net[-1].bias)
        ...
        if use_additive:
            ...
            if zero_init:
                torch.nn.init.zeros_(self.beta_net[-1].weight)
                torch.nn.init.zeros_(self.beta_net[-1].bias)

    def forward(
        self, features: torch.Tensor, charge: torch.Tensor
    ) -> torch.Tensor:
        """
        Args:
            features: [n_nodes, irreps_dim] -- full equivariant feature tensor
            charge: [n_nodes, 1] -- per-node charge density
        Returns:
...
class RealAgnosticInteractionBlock(InteractionBlock):
    ...
    def forward(
        ...
        charge_conditioning: Optional[torch.Tensor] = None,
        ...
        # FiLM conditioning: modulate message after aggregation
        if charge_conditioning is not None and hasattr(self, "charge_film"):
            message = self.charge_film(message, charge_conditioning)
